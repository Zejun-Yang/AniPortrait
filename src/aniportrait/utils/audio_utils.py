...
class DataProcessor:
    ...
    def extract_feature(self, audio_path):
        ...
        input_value = np.squeeze(
            self._processor(speech_array, sampling_rate=sampling_rate).input_values
        )
        return input_value


def prepare_audio_feature(
    wav_file, fps=30, sampling_rate=16000, wav2vec_model_path=None
):
    ...
    return {"audio_feature": input_value, "seq_len": seq_len}
